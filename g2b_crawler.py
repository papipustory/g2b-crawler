# g2b_crawler.py - Streamlit Cloud 배포용 G2B 제안공고 크롤러 (속도+안정성 개선)
import asyncio
...
async def run_crawler_async(query="컴퓨터", browser_executable_path=None):
    """
    Streamlit Cloud 환경 최적화 G2B 제안공고 크롤러 (wait_for_selector 사용/속도 개선)
    ...
    try:
        async with async_playwright() as p:
                ...
                for _ in range(5):
                    ...
                    for btn in btns:
                        try:
                            ...
                            continue
                    # 닫을 게 더 있나? wait_for_selector(500ms)
                    try:
            ...
            # 공고목록 팝업 페이지 등장 대기 (표시 5초 한도)
            # 3개월, 검색 등 요소가 등장할 때까지 대기 (동적페이지용)
            try:
                await page.wait_for_selector('input[title="3개월"], input[type="radio"]', timeout=5000)
            except PlaywrightTimeout:
                pass

            # 검색어 입력 (wait_for_selector로 입력창이 보일 때까지)
            input_success = False
            try:
                await page.wait_for_selector('td[data-title="제안공고명"] input[type="text"], input[placeholder*="제안공고명"], input[title*="제안공고명"]', timeout=5000)
            ...
            try:
                await page.wait_for_selector('input[title="3개월"], input[type="radio"]', timeout=2000)
                try:
                    await page.click('input[title="3개월"]', timeout=2000)
                except Exception:
                    await page.evaluate("""
            ...
            # 표시수 100개
            try:
                await page.wait_for_selector('select[id*="RecordCountPerPage"]', timeout=1500)
