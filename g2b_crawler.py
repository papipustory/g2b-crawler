# g2b_crawler.py - Streamlit Cloud 최적화 + 속도개선 (최소 sleep)
import asyncio
        ...
        async with async_playwright() as p:
            ...
            if browser_executable_path:
                browser = await p.chromium.launch(
                    ...
                    args=browser_args, timeout=16000
                )
            else:
                browser = await p.chromium.launch(
                    headless=True, args=browser_args, timeout=16000
                )
            ...
            await context.add_init_script("""
                ...
                Object.defineProperty(navigator, 'plugins', {get: () => [{name: 'Chrome PDF Plugin', filename: 'internal-pdf-viewer'}, {name: 'Chrome PDF Viewer', filename: 'mhjfbmdgcfjbbpaeojofohoefgiehjai'}, {name: 'Native Client', filename: 'internal-nacl-plugin'}],});
            ...
            for attempt in range(2):
                ...
                for header in popup_headers:
                    try:
                        ...
                        """)
                        if not await popup_container.is_visible():
                            continue
            ...
            await page.evaluate("""
                (() => {
                    document.querySelectorAll('button.w2window_close').forEach(btn => {if (btn && btn.offsetParent !== null) btn.click();});
                    document.querySelectorAll('input[type="button"][value="닫기"]').forEach(btn => {if (btn && btn.offsetParent !== null) btn.click();});
                })();
            """)
            ...
            if not button_clicked:
                try:
                    await page.click('xpath=//a[contains(text(), "제안공고목록")]', timeout=2000)
                    button_clicked = True
                except:
                    pass
            if not button_clicked:
            ...
            try:
                ...
                pass
            try:
                await page.click('input[value="검색"]', timeout=1200)
            except:
                await page.evaluate("""
